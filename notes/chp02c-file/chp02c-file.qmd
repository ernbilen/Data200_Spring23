...
-   Operating systems, such as Windows, MacOS, or Linux have similar
    ...
    -   Files are store as sequences of bits (0’s and 1’s) forming
        bytes (8-bits).
...
# Three Main Parts of Most Files

::: {.column width="45%"}
![](assets/img/image3.png){width="3in"}
:::
...
3.  **End of file:** special character that indicates the end of the
    file.
:::
...
-   Operating systems utilize a hierarchical organization to organize
    files.
    -   This hierarchy defines a tree , where the root and interior
        nodes of the tree provide the abstraction of a directory or
        folder .
    -   Each folder can have contents, which are the children of the
        folder node in the tree.
    -   The child contents of a folder can consist of zero or more files
        as well as zero or more (sub)folders.
    -   A leaf is a node with no children. As internal node is a node
        that is not a leaf. A file must be a leaf of the tree, as it
        cannot have any subordinate folders or files.

# 

![](assets/img/image4.png)

-   This is abstracted from a Linux system.
...
-   Here we focus on the user $\texttt{alice}$ and the subtree in the
    file system that represents her home directory, which is the portion
    of the file system she sees when see logs in, and when she starts a
    File Explorer.
...
-   For example, files and folders can also be accessed directly using
    applications such as Terminal on a MacOS or Command Prompt on
    Windows.
...
-   The path "`../Pictures/profile.jpg`" defines the traversal
    `/home/alice/Pictures/profile.jpg` :wrench:notice "Documents" is
    absent.
...
![](assets/img/yoda.jpg){.fragment fragment-index=2 .absolute bottom="360" right="-70"
    right="100" width="4in" height="3in"}
:::
...
- The `open(.)` function creates and returns a file object, which can
    be used to call other file support methods. <br>
$\hspace{1cm}$  `file = open('hello.txt', 'r')` <br> **<u>hello.txt</u>** <br>
Hello World!
:::
...
-   The OS module in Python provides functions for interacting with the
    ...
    `os.chdir('DataFolder')` Changes the current working directory <br>
...
-   Below we demonstrate a simple try-except block that can handle the
    ...
    being opened. 
![](assets/img/image10.png){width=5in}
...
-   Once you've opened a file, you'll want to read or write to it .
    -   We must understand the concept of the current position in the
        file, which is an integer that operates much like a cursor in a
        word processor. It indicates where the next operation will
        occur.
    -   The file position starts at 0, which is the position at the very
    ...
    -   In this way, a subsequent read-type operation will continue from
        the point where the last read finished.
...
# Another Example
<u>twolines.txt</u> <br>
<font face="monospace"> First line </font> <br>
...
**Output:** <br>
<font face="monospace"> This is the first line.</font> <br>
<font face="monospace"> This is the second line.  </font> <br>
<font face="monospace"> 'This is the first line.\\nThis is the second line.'</font>
...
-   If we are processing a file by lines and want to remove whitespace
    ...
    -   `strip(.)` to eliminate both leading and trailing whitespace in a
        string
...
```
<u>Output:</u> <br>
<font face="monospace">\['Python', 'is', 'a', 'fun', 'programming', 'language'\] </font>
...
-   A very common error in file processing is to fail to include the
    ...
    -   There are two ways that you can use to ensure that a file is
        ...
        -   `with` is typically more preferred.
...
`try-finally` block, check out the webpage [Python
Exceptions: An Introduction](https://realpython.com/python-exceptions/)
...
+ Important::warning: It is highly recommended that you use the `with` statement as much as possible, as it allows for cleaner code and makes handling any unexpected errors easier for you.

# with Continued

-   Most likely, you'll also want to use the second positional
    argument, `mode`.
...
::: {.column width="45%"}
::: small
Note that the `print(.)` function stops with a newline. By default, the value of this parameter is `\n`, but you can end a print statement with any
character. Here, we are ending it with no characters (this prevents
extra carriage returns in the output) Try playing around with this option and see what it gets returned to you.
:::
:::
...
-   We begin with the assumption that our file has one datum per
        line, and that the set of lines form a coherent collection.
...
::: monospace
<u>Baby_2010_female_name.txt</u> <br>
Isabella  <br>
Sophia  <br>
Emma  <br>
Olivia <br>
...
-   Our first objective is to create a function that processes the
        file and returns a single Python list with a name per entry in
        the file. 
...
- Note that the for-loop continues as long as there are lines, i.e., until it has reached the end-of-file.


- This method is typically quicker and more memory efficient. Therefore,
it is the suggested way to process a file.
